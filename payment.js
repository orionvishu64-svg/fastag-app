...
// --- Delhivery Pincode Serviceability (Prepaid-only version) ---
(function(){
  async function checkPincodeServiceability(pincode) {
    ...
    try {
      const res = await fetch("pincode_serviceability.php", {
        method:"POST",
        headers:{ "Content-Type":"application/json" },
      ...
      if (data && data.success && data.data) {
        ...
        return { ok:false, message:data?.message || "Not serviceable" };
      }
    } catch(e) {
  ...
  if (pincodeInput) {
    ...
    pincodeInput.addEventListener("blur", async ()=>{
      ...
      lastCheck = result;
      const msgBox = document.getElementById("pincode-msg") || (() => {
        const div = document.createElement("div");
        div.id = "pincode-msg";
        div.style.fontSize="0.9em";
        div.style.marginTop="6px";
        pincodeInput.parentElement.appendChild(div);
        return div;
      })();
      if (result.ok) {
        msgBox.textContent = `✅ Serviceable. Expected TAT: ${result.data.tat || result.data.tat_days || 'N/A'} days`;
        msgBox.style.color = "#0a7";
      } else {
        msgBox.textContent = `❌ ${result.message}`;
        msgBox.style.color = "#b00";
      }
    });

    // Block save if pincode not serviceable
    const saveAddressBtn = document.getElementById("save-address");
    if (saveAddressBtn) {
      saveAddressBtn.addEventListener("click", async (ev)=>{
        const code = pincodeInput.value.trim();
        const result = await checkPincodeServiceability(code);
        if (!result.ok) {
          ev.preventDefault();
          alert("This pincode is not serviceable by our courier partner.");
          return false;
        }
      });
    }
...
// Expose origin pincode (warehouse) — set your origin here or list from server
const ORIGIN_PIN = '560001'; // change to your warehouse pincode

async function fetchTAT(dest_pin, weight=1.0, mode='E') {
  try {
    const resp = await fetch('expected_tat.php', {
      method: 'POST',
      headers: {'Content-Type':'application/json'},
      body: JSON.stringify({ origin_pin: ORIGIN_PIN, dest_pin, weight, mode })
    });
...
// Example: after pincode validated (adapt to your existing pincode callback)
async function showTatForPincode(pincode, elementToUpdate) {
  const r = await fetchTAT(pincode);
  if (r && r.tat != null) {
    elementToUpdate.innerText = `Estimated delivery: ${r.tat} day(s)` + (r.expected_date ? ` (by ${r.expected_date.split('T')[0]})` : '');
  } else {
    elementToUpdate.innerText = ''; // or show fallback
  }
}