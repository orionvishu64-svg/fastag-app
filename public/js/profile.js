  ...
  if (addPartnerBtn && addPartnerHost && partnersGrid) {
    addPartnerBtn.addEventListener('click', function () {
      ...
      chooser.innerHTML = `
        ...
        <div style="margin-bottom:8px" class="helper">
        ...
        <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;">
          ...
          <button class="btn btn-secondary" data-type="gv"      type="button">GV Partner ID</button>
          <button class="btn-cancel" style="margin-left:auto" type="button">Cancel</button>
        </div>`;
      ...
      chooser.querySelectorAll('[data-type]').forEach(button => {
        button.addEventListener('click', function () {
          ...
          if (type === 'gv') {
            editor.innerHTML = `
              <div class="editor-head">
                <h4>Add GV Partner</h4>
              </div>
              <form class="inline-edit-form">
                <div>
                  <label>GV Partner ID
                    <input name="gv_partner_id" placeholder="e.g. GV-ABC123">
                  </label>
                </div>
                <div>
                  <label>Name (optional)
            ...
            editor.innerHTML = `
              ...
              <form class="inline-edit-form">
                <div class="row">
                  <div class="col">
                    <label>Bank Name
                      <input name="bank_name" placeholder="e.g. HDFC/ICICI/SBI">
                    </label>
                  </div>
                  <div class="col">
                    <label>Partner ID
                      <input name="partner_id" placeholder="e.g. ABC12345">
                    </label>
                  </div>
                </div>
                <div>
          ...
          form.addEventListener('submit', async function (e) {
            e.preventDefault();
            const table = (this.elements['table'] && this.elements['table'].value) || 'partners';
            const fd    = new FormData();
            ...
            try {
              ...
              if (res && res.success && res.partner) {
                ...
                let title;
                if (table === 'gv_partners') {
                  title = `GV - ${p.gv_partner_id || ''}${p.name ? ' — ' + p.name : ''}`;
                } else {
                  title = `${p.name || ''} — ${p.partner_id || ''}${p.bank_name ? ' (' + p.bank_name + ')' : ''}`;
                }
                ...
                card.innerHTML = `
                  ...
                  <div class="card-actions">
                    <button class="btn btn-secondary edit-partner-btn"
                            data-id="${p.id}"
                            data-table="${table}"
                            data-bank_name="${(p.bank_name || '').replace(/"/g, '&quot;')}"
                            data-partner_id="${(p.partner_id || '').replace(/"/g, '&quot;')}"
                            data-gv_partner_id="${(p.gv_partner_id || '').replace(/"/g, '&quot;')}"
    ...
    btn.addEventListener('click', function (e) {
      ...
      editor.innerHTML = `
        ...
        <form class="inline-edit-form">
          <div>
            <label>House / Flat No
              <input name="house_no" value="${house.replace(/"/g, '&quot;')}">
            </label>
          </div>
          <div>
            <label>Landmark
              <input name="landmark" value="${landmark.replace(/"/g, '&quot;')}">
            </label>
          </div>
          <div class="row">
            <div class="col">
              <label>City
                <input name="city" value="${city.replace(/"/g, '&quot;')}">
              </label>
            </div>
            <div class="col">
              <label>Pincode
                <input name="pincode" value="${pincode.replace(/"/g, '&quot;')}">
              </label>
            </div>
          </div>
          <div class="editor-actions">
  ...
  if (addAddressBtn && addAddressHost) {
    addAddressBtn.addEventListener('click', function () {
      ...
      editor.innerHTML = `
        ...
        <div class="helper">
          Save a new shipping address for your orders.
        </div>
        <form class="inline-edit-form">
          <div>
            <label>House / Flat No
              <input name="house_no" placeholder="e.g. 24B, Green Apartments">
            </label>
          </div>
          <div>
            <label>Landmark
              <input name="landmark" placeholder="Near City Mall">
            </label>
          </div>
          <div class="row">
            <div class="col">
            ...
            <div class="col">
              <label>Pincode
                <input name="pincode" placeholder="e.g. 800001">
              </label>
            </div>
          </div>
          <div class="editor-actions">
