...
(function () {
  ...
  function pickLogo(product) {
    ...
    return cand
      ? normalizePath(cand)
      : svgPlaceholder(product.bank || product.name || "?");
  }
  ...
  function addToCartLocal(p, qty) {
    ...
    if ("quantity" in item)
      item.quantity = Math.max(1, Number(qty || item.quantity || 1));
  ...
  function initSheet() {
    ...
    function wireAdd(product) {
      ...
      addBtn.addEventListener("click", function () {
        ...
        if (typeof window.addToCart === "function") {
          try {
            window.addToCart({ ...product, qty: q });
          } catch {}
        } else {
    ...
    function show(product) {
      ...
      try {
        ...
        bankCat.textContent = `${product.bank || ""} • ${
          product.category || ""
        }`;
        activation.textContent =
          product.activation != null ? `₹${product.activation}` : "—";
        balance.textContent =
          product.balance != null ? `₹${product.balance}` : "—";
        security.textContent =
          product.security != null ? `₹${product.security}` : "—";
        tagcost.textContent =
          product.tagcost != null ? `₹${product.tagcost}` : "—";
  ...
  async function loadAndRender(opts = {}) {
    const bank =
      opts.bank === undefined ? document.body?.dataset?.bank : opts.bank;
    ...
    try {
      ...
      container.innerHTML =
        '<div class="notice-card">Failed to load products.</div>';
    }
  }
  ...
  function uiToParams() {
    ...
    const categoryValRaw =
      document.getElementById("categoryFilter")?.value || "";
    const q = document.getElementById("searchInput")?.value?.trim() || "";
    const bank = bankValRaw && bankValRaw !== "all" ? bankValRaw : null;
    const category =
      categoryValRaw && categoryValRaw !== "all" ? categoryValRaw : null;
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    if (search)
      search.addEventListener(
        "input",
        debounce(() => reloadAndUpdateUI(), 300)
      );
  ...
  document.addEventListener(
    ...
    function (e) {
      ...
      try {
        ...
        if (idx !== null && window._productIndex && window._productIndex[idx])
          prod = window._productIndex[idx];
      } catch {
        prod = null;
      }
      if (!prod && card.dataset && card.dataset.product) {
        try {
          prod = JSON.parse(card.dataset.product);
        } catch {
          prod = null;
        }
      }
