// profile.js — inline slide-down editor + AJAX profile/address + ADD PARTNER (no overlay)
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (profileForm) {
    profileForm.addEventListener('submit', async function (e) {
      ...
      fd.append('action', 'profile_update'); // server updates name + phone (email readonly)
  ...
  // --------------------------------------------
  // 4) ADD PARTNER (inline form at top)
  // --------------------------------------------
  ...
  if (addBtn && addHost && grid) {
    addBtn.addEventListener('click', function () {
      ...
      editor.innerHTML = `
        <form class="inline-edit-form">
          <div><label>Bank Name
            <input name="bank_name" placeholder="e.g. HDFC/IDFC/SBI">
          </label></div>
          <div><label>Partner ID
      ...
      editor.querySelector('.inline-edit-form').addEventListener('submit', async function (e) {
        ...
        const nm   = this.elements['name'].value.trim();
        if (!bank || !pid) {
        ...
        try {
          ...
          if (json.success && json.partner) {
            // Build a new card and prepend to grid
            const p = json.partner; // {id, bank_name, partner_id, name, created_at}
            const card = document.createElement('div');
            card.className = 'partner-card';
            card.setAttribute('data-row-id', p.id);
            card.setAttribute('data-table', 'partners');
            card.innerHTML = `
              <div class="partner-title">${p.bank_name || ''} — ${p.partner_id || ''}${p.name ? ' (' + p.name + ')' : ''}</div>
              ...
              <div class="card-actions">
                <button class="btn btn-secondary edit-partner-btn"
                        ...
                        data-name="${(p.name||'').replace(/"/g,'&quot;')}">Edit</button>
            ...
            // Prepend
            grid.prepend(card);
            // Bind edit for the new card
            const newEditBtn = card.querySelector('.edit-partner-btn');
            if (newEditBtn) { bindEditButton(newEditBtn); newEditBtn._editBound = true; }

            // close the add editor
            editor.style.height = editor.scrollHeight + 'px';
            requestAnimationFrame(() => {
              editor.style.transition = 'height 220ms ease';
              editor.style.height = '0px';
            ...
            flash(json.message || 'Partner added', true);
          } else {
            flash(json.message || 'Add failed', false);
          }
        } catch (err) {
          console.error(err);
          flash('Network/server error', false);
